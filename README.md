...
6. Offers 128 topic ideas across 16 optional niches while keeping free-form topic input available.
7. Generates one visual per card with GPT Image.
8. Composites and saves final 4:5 PNGs plus the complete post package.
...
The Studio includes four writing-tone presets: `casual` (friendly informal), `polite` (friendly polite), `expert` (calm expert), and `punchy` (short and direct). The default is `casual`.
...
6. 16개 니치의 키워드 예시 128개를 제공하며, 아무것도 선택하지 않고 주제를 직접 입력할 수도 있습니다.
7. GPT Image로 카드마다 하나의 배경 이미지를 생성합니다.
8. 텍스트를 합성한 최종 4:5 PNG와 게시 패키지를 함께 저장합니다.
...
Studio에서 `친근한 반말(casual)`, `친근한 존댓말(polite)`, `차분한 전문가(expert)`, `짧고 강한 직설(punchy)` 중 글 말투를 고를 수 있습니다. 기본값은 `casual`입니다.
